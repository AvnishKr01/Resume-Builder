...
export const TemplateCard = ({ thumbnailImg, isSelected, onSelect }) => {
  return (
    ...
    cursor-pointer transition-all duration-500 hover:scale-105 hover:shadow-lg rounded-3xl ${isSelected ? 'border-violet-500 shadow-lg shadow-violet-500/20 bg-violet-50 ' : 'border-gray-200 hover:border-violet-300'}`
    } onClick={onSelect}>
      {thumbnailImg ? (
        <div className="relative w-full h-full overflow-hidden">
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-white/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"/>
            ...
            {/* Hover Effect  */}
            <div className="absolute inset-0 bg-gradient-to-r from-violet-100/30 to-transparent opacity-0 *:group-hover:opacity-100 transition-opacity duration-300">
            </div>
          </div> 
    ) : (
