...
import { fixTailwindColors } from '../util/colors'
...
const EditResume = () => {
  ...
  const fetchResumeDetailsById = async () => {
    try {
      const response = await axiosInstance.get(AUTH_PATHS.RESUME.GET_BY_ID(resumeId))
  ...
  const uploadResumeImages = async () => {
    try {
      ...
      const uploadResponse = await axiosInstance.put(
        AUTH_PATHS.RESUME.UPLOAD_IMAGES(resumeId),
        formData,
        {
      ...
      const { thumbnailLink, thumbnail } = uploadResponse.data
      await updateResumeDetails(thumbnailLink || thumbnail || "")
  ...
  const updateResumeDetails = async (thumbnailLink) => {
    try {
      ...
      await axiosInstance.put(AUTH_PATHS.RESUME.UPDATE(resumeId), {
        ...resumeData,
  ...
  // DELETE FUNCTION TO ANY RESUME

  const handleDeleteResume = async () => {
    try {
      setIsLoading(true)
      await axiosInstance.delete(AUTH_PATHS.RESUME.DELETE(resumeId))
